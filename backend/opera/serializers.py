...
class PerformanceReviewConclusionSerializer(serializers.ModelSerializer):
    stage_performance_display = serializers.CharField(source='get_stage_performance_display', read_only=True)
    rhythm_stability_display = serializers.CharField(source='get_rhythm_stability_display', read_only=True)
    teacher_name = serializers.CharField(source='teacher.name', read_only=True, allow_null=True)

    class Meta:
        model = PerformanceReviewConclusion
        fields = '__all__'


class MemberSelfReviewSerializer(serializers.ModelSerializer):
    member_name = serializers.CharField(source='member.name', read_only=True)

    class Meta:
        model = MemberSelfReview
        fields = '__all__'
...
class ImprovementTaskSerializer(serializers.ModelSerializer):
    ...
    aria_name = serializers.SerializerMethodField()

    class Meta:
        model = ImprovementTask
        fields = '__all__'

    def get_aria_name(self, obj):
        if obj.review_item:
            return obj.review_item.aria.name
        return None


class PerformanceReviewItemSerializer(serializers.ModelSerializer):
    aria_name = serializers.CharField(source='aria.name', read_only=True)
    aria_id = serializers.IntegerField(source='aria.id', read_only=True)
    role_type_display = serializers.SerializerMethodField()
    conclusions = PerformanceReviewConclusionSerializer(many=True, read_only=True)
    self_reviews = MemberSelfReviewSerializer(many=True, read_only=True)
    improvement_tasks = ImprovementTaskSerializer(many=True, read_only=True)
    has_conclusion = serializers.SerializerMethodField()
    self_review_count = serializers.SerializerMethodField()
    improvement_task_count = serializers.SerializerMethodField()
    ...
    def get_has_conclusion(self, obj):
        return obj.conclusions.exists()

    def get_self_review_count(self, obj):
        return obj.self_reviews.count()

    def get_improvement_task_count(self, obj):
        return obj.improvement_tasks.count()


class PerformanceReviewSerializer(serializers.ModelSerializer):
    program_name = serializers.CharField(source='program.name', read_only=True)
    status_display = serializers.CharField(source='get_status_display', read_only=True)
    archive_version = serializers.CharField(source='archive.version', read_only=True, allow_null=True)
    item_count = serializers.SerializerMethodField()
    conclusion_count = serializers.SerializerMethodField()
    self_review_total = serializers.SerializerMethodField()
    improvement_task_count = serializers.SerializerMethodField()
    ...
    def get_conclusion_count(self, obj):
        return PerformanceReviewConclusion.objects.filter(review_item__review=obj).count()

    def get_self_review_total(self, obj):
        return MemberSelfReview.objects.filter(review_item__review=obj).count()
    ...
    def get_completion_rate(self, obj):
        ...
        completed_items = 0
        for item in obj.items.all():
            if item.conclusions.exists():
                completed_items += 1
        return round(completed_items / total_items * 100, 1)


class CreatePerformanceReviewRequestSerializer(serializers.Serializer):
    program_id = serializers.IntegerField()
    name = serializers.CharField(max_length=200)
    performance_date = serializers.DateField()
    archive_id = serializers.IntegerField(required=False, allow_null=True)
    actual_performance_note = serializers.CharField(required=False, allow_blank=True)


class ConvertToTaskRequestSerializer(serializers.Serializer):
    review_item_id = serializers.IntegerField()
